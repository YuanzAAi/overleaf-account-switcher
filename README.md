...
<p align="center"><strong>MOGA · Overleaf 现代工作流</strong></p>
<p align="center">连通 coding agent，打通项目迁移，让大型论文不再困于编译超时。</p>
...
随着 ChatGPT、Claude 等产品背后的大模型不断发展，LaTeX 排版和编译的技术门槛正在降低。基于这些模型的 coding agent 已经能帮我们配置 TeX 环境、修改论文、排查报错，再把源码编译成 PDF。很多时候，你只需要说明想做什么；在获得授权后，agent 还能直接处理本地的文献、实验数据和图片，不必先把材料一份份上传。

而在这之前，Overleaf 已经替很多人省去了不少折腾：
...
这些优点今天仍然成立。但写作流程已经不只在浏览器里了：agent 可以按项目需要配置 pdfLaTeX、XeLaTeX 或 LuaLaTeX，用 latexmk 完成编译，还能结合本地材料直接修改文章。**写 LaTeX 越来越方便，Overleaf 也该接上这套现代工作流。**
...
**Overleaf Account Switcher 是一个本地优先的 Overleaf 账号与项目工作台。** 通过 [overleaf-skills](https://github.com/YuanzAAi/overleaf-skills)，让 Codex、Claude Code 处理本地材料、编辑 Overleaf 项目、调用云端编译并取回 PDF；需要换号时，把项目迁移、浏览器会话与 skills 凭据一起衔接起来。云端协作继续用，agent 也能直接参与，不必再手动来回搬家。
...
- **把换号和项目迁移连起来。** 需要更换有合适编译权益的账号时，先处理项目归属与访问权限，再继续写作。
- **浏览器与 skills 使用同一套账号。** 无感换号时按需同步 Cookie 和 Git token，不用分别手动配置。
- **Cookie 失效后继续处理。** 已保存密码的账号可尝试重新登录恢复，再继续原任务；遇到 reCAPTCHA 或邮箱验证时等待你处理。
- **桌面和 Web UI 共用一套功能。** Windows、macOS 与 Docker 使用相同的服务和界面，操作习惯保持一致。
...
<p align="center">
  <a href="docs/architecture.html"><img src="docs/images/architecture.png" alt="Overleaf Account Switcher 架构与功能联动" width="1200"></a>
</p>
...
账号数据保存在本机，密码、Cookie、Git token 和卡片敏感字段由密钥库保存。换号成功后可同步 skills 凭据；agent 再通过 Cookie 读取和编译项目，通过 Git 提交修改、查看历史。桌面版在本机运行服务，Docker 在容器中运行服务，coding agent 仍在宿主机使用。

[交互原理图](docs/architecture.html) · [图表源文件](docs/architecture.json)
...
Cookie 用于读取、编译与下载；编辑和历史操作使用 Git，需要相应的 [Overleaf Git 集成权限](https://docs.overleaf.com/integrations-and-add-ons/git-integration-and-github-synchronization/git-integration)。账号同步需要 Cookie 与 Git token，缺少时会提示并保留原 skills 状态。
...
<p align="center">
  认可社区：<a href="https://linux.do"><img src="https://cdn3.ldstatic.com/original/4X/d/1/4/d146c68151340881c884d95e0da4acdf369258c6.png" alt="" height="24"></a> <a href="https://linux.do"><strong>LINUX DO</strong></a>
</p>
